...
local mappings = {
  ...
  ["m"] = { function ()
    vim.ui.input({ prompt = "Enter macro name: " }, function (input)
      local macro = input
      vim.cmd(("cdo execute 'norm! @%s'|update"):format(macro))
    end)
  end, "Execute Macro on Quickfix" },
  ...
  r = {
    ...
    e = { "<cmd>TestEdit<cr>", "Test Edit" },
    n = { "<cmd>TestEdit<cr>", "Test Nearest" },
    l = { "<cmd>TestEdit<cr>", "Test Last" },
    v = { "<cmd>TestEdit<cr>", "Test Visit" },
  },
